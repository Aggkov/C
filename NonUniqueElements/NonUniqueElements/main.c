...
int findNonUniqueElements(int arr[], rsize_t size) {
	...
	for (int i = 0; i < size; i++) {
		...
		for (int j = i + 1; j < size; j++) {
			if (arr[i] == arr[j] && !contains(haveSeenBefore, arr[i], SIZE)) {
	...
	printf("Non Unique elements are: \n");
	for (int i = 0; i < size; i++) {
		printf("%d ", haveSeenBefore[i]);
	}
	printf("\n");
	return count;
}
